Feature: Party Order as a Consultant
  ...
  Scenario: Add Hostess Credit
    ...
    Then I verify my party order totals,Retail:"$280.00",Volume:"$255.00",Subtotal:"$255.00",ShipHand:"$24.40",Tax:"$21.00",Total:"$300.40"
  ...
  Scenario: Add exclusive hostess offer
    ...
    Then I verify my party order totals,Retail:"$345.00",Volume:"$345.00",Subtotal:"$320.00",ShipHand:"$29.60",Tax:"$26.17",Total:"$375.77"
  ...
  Scenario: Add Hostess Half Price
    ...
    Then I verify my party order totals,Retail:"$363.00",Volume:"$255.00",Subtotal:"$329.00",ShipHand:"$30.32",Tax:"$26.85",Total:"$386.17"
  ...
  Scenario: Add 31gives
    ...
    Then I verify my party order totals,Retail:"$364.35",Volume:"$255.00",Subtotal:"$330.35",ShipHand:"$30.32",Tax:"$26.85",Total:"$387.52"
    And I verify the Party Hostess Totals Hostess Plan Base Amt "$210.00" Hostess Half Price "1 / 1" Hostess Credit "$25.00 / $0.00" Exclusive Hostess Offer "1 / 1" Hostess Overage "$45.00"

  Scenario: Remove a Product from Order
    When I continue my order
    And I remove an item from my party order
    Then I verify my party order totals,Retail:"$314.13",Volume:"$230.00",Subtotal:"$305.13",ShipHand:"$28.32",Tax:"$24.96",Total:"$358.41"
  ...
  Scenario: Change Shipping Information on Guest
    ...
    Then I verify my party order totals,Retail:"$364.36",Volume:"$255.00",Subtotal:"$330.36",ShipHand:"$34.32",Tax:"$29.84",Total:"$394.52"
    And I verify the Party Hostess Totals Hostess Plan Base Amt "$210.00" Hostess Half Price "1 / 1" Hostess Credit "$25.00 / $0.00" Exclusive Hostess Offer "1 / 1" Hostess Overage "$45.00"

  Scenario: Change Personalization
    When I continue my order
    And I change a personalization option
    Then I verify my party order totals,Retail:"$374.36",Volume:"$265.00",Subtotal:"$340.36",ShipHand:"$35.12",Tax:"$30.59",Total:"$406.07"
